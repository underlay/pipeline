...
const { inputSchemaPaths: { input: inputSchemaPath }, inputInstancePaths: { input: inputInstancePath }, state, } = params;
...
const inputSchemaEntry = content.entry({ name: "index.schema", size: schemaStat.size }, (err) => {
    if (err) {
        throw err;
    }
    else {
        const inputInstanceStream = createReadStream(inputInstancePath);
        const inputInstanceEntry = content.entry({ name: `instances/${uuid()}.instance`, size: instanceStat.size }, (err) => {
            if (err) {
                throw err;
            }
            else {
...
const res = await fetch(`http://localhost:8086/api/v0/collection?${host}&${id}`, {
    method: "POST",
